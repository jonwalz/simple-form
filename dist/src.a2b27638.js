...
theForm.addEventListener("submit", function (event) {
  ...
  */
  ...
  var strain = document.querySelector("#dropdown").value; // We have now stored the values of the fields in variables
  ...
  console.log("Data: ", data); // Now we prepare it for submitting by using the URLSearchParams api
  // We first find the url we want to post to. In this case:

  var rawUrl = "https://script.google.com/macros/s/AKfycbyP0re5vf0LmzUNfb4GbSocMBMcG-HtAmaELvZHTGzgZNz6B6Sv/exec"; // Then we use the URL api to construct the URL, so that we are able to use URLSearchParams:

  var url = new URL(rawUrl); // (This is new shit so don't expect it to work in every browser. Tell people to stop using internet explorer)
  // Set the search params to the URL

  url.search = new URLSearchParams(data);
  console.log("Url: ", url); // Now we use the build in fetch api to make a get request and send the data
